...
const char *WCCOAJavaManager::ManagerClassName = "com/etm/api/base/JManager";
const char *WCCOAJavaManager::HotLinkWaitForAnswerClassName = "com/etm/api/base/JHotLinkWaitForAnswer";
...
void WCCOAJavaManager::javaInitialize(JNIEnv *env, jobject obj){
	...
	midJavaHotlink = env->GetMethodID(javaManagerClass, "callbackHotlink", "(IILcom/etm/api/var/DpIdentifierVar;Lcom/etm/api/var/Variable;)I");
	...
	midJavaAnswer = env->GetMethodID(javaManagerClass, "callbackAnswer", "(IILcom/etm/api/var/DpIdentifierVar;Lcom/etm/api/var/Variable;)I");
...
void WCCOAJavaManager::doReceive(SysMsg &sysMsg)
{
	...
	if (g_env == nil || g_obj == nil) {
		std::cout << "doReceiveSysMsg without java env!" << std::endl;
	}
	else
	{
...
void WCCOAJavaManager::doReceive(DpMsg& dpMsg)
{
	...
	jboolean handled = false;
	if (g_env == nil || g_obj == nil) {
		std::cout << "doReceiveDpMsg without java env!" << std::endl;
	} 
	else 
	{
		//std::cout << dpMsg << std::endl;
		handled = g_env->CallBooleanMethod(g_obj, midDoReceiveDpMsg, (jlong)(&dpMsg));		
		//std::cout << "doReceiveDpMsg return = " << ret << std::endl;
	}

	if (!handled) Manager::doReceive(dpMsg);
}

void WCCOAJavaManager::doReceiveParent(SysMsg &msg) {
	Manager::doReceive(msg);
}

void WCCOAJavaManager::doReceiveParent(DpMsg &msg) {
	Manager::doReceive(msg);
}
...
void WCCOAJavaManager::javaSetHdlCptr(JNIEnv *env, jobject jHdl, jlong cptr)
{
	...
	jclass cls = env->FindClass("com/etm/api/base/JHotLinkWaitForAnswer");
