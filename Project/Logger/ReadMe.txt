This Data Logger for WinCC OA is a write only data logger from the SCADA System WinCC OA to different database. In this example MongoDB is used to store the data, other database backends like Elasticsearch or Cassandra are also available. The Logger consists of a Frontend and a Backend Manager. The Frontend Manager will collect the values from WinCC OA by simple dpQueryConnects (which can be configured in the frontend.config file, also multiple connects can be definied). It will pack single value changes to blocks and transfer that blocks by ZeroMQ to Backend Managers. The Backend Manager(s) will get the blocks from ZeroMQ from the Frontend and will write the values to the database. Multiple Backend Managers can be connected to one single Frontend Manager. The Backend Manager will buffer values in memory and/or on disk if the database is partly too slow or unavailable.
